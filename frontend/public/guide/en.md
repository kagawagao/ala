...
5. **Choose an AI model** – Switch the active AI model from the header selector, or go to Model Management to add custom models.
...
- After upload, files enter a **pending** state — the AI agent loads them on demand, avoiding unnecessary pre-parsing.
...
- **Lazy loading** — Log files are uploaded to temporary storage and filtered on-demand via streaming. Large files load progressively without blocking the UI.
...
- Use **SQL queries** (Python Perfetto API) in the AI chat to run custom queries against the trace for advanced analysis.
...
1. **Configure AI** – Open **Model Management** (grid icon in the header) to browse built-in models or add custom ones.
   ALA supports Anthropic Claude, OpenAI-compatible APIs, and any provider with an OpenAI-compatible endpoint. Each model can have its own API endpoint, key, temperature, and thinking configuration. Switch the active model quickly from the header selector.
2. **Ask questions** – Type in the AI chat panel on the right. The assistant can analyze filtered log entries, loaded traces, PCAP/HCI data, and local file/directory contents.
3. **Agent mode** – Enable agent mode to let the AI actively query log overviews, search entries, inspect trace processes, run SQL queries on traces, and filter PCAP/HCI packets.
4. **Attach context** – Use the context selector to focus the assistant on logs, trace data, PCAP data, HCI data, a local file, a local directory, or general questions.
...
6. **Extended thinking** – Enable "Thinking mode" for models that support it (e.g., Claude) for deeper reasoning with configurable token budgets. Three modes: Off, Auto (hidden), On (visible).
7. **Download conversation** – Click the download button in the chat panel to export the full conversation as a Markdown file.
...
1. Open the **Model Management** page (grid icon in the header).
2. **Built-in Models** — Browse pre-configured common AI models. Click one to auto-fill the configuration fields.
3. **Custom Models** — Add any OpenAI-compatible model with a display name, model ID, API endpoint, provider label, and optional extended thinking support.
4. **Activate/Deactivate** — Toggle whether a model appears in the header model selector. Only activated models with configured API keys are shown.
5. **Model Selector** — Quickly switch the active AI model from the header on the main page without entering the management page.
6. **API Key** — Each model's API key is stored separately in your browser's localStorage and is never synced to the backend.

---

## System Settings

1. Open the **System Settings** page (gear icon in the header) to configure application-level global preferences.
2. Currently supports configuring default download path and other options; more settings will be added in future releases.
...
- Uploaded files enter a **pending** state — the AI agent loads them on demand to avoid unnecessary preprocessing.
