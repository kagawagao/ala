...
const genericLog = readFileSync('test-generic-log.txt', 'utf-8');
...
const androidParsed = analyzer.parseLog(androidLog);
...
const genericParsed = analyzer.parseLog(genericLog);
...
const infoParsed = analyzer.parseLog(infoLog);
const errorParsed = analyzer.parseLog(errorLog);
const warnParsed = analyzer.parseLog(warnLog);
